...
if [ -f "/.influxdb_configured" ]; then
    ...
    do
        ...
        curl -s -X POST  $(echo ${API_URL}'/query?u=root&p=root') --data-urlencode "q=CREATE DATABASE ${x}"
    done
    echo ""
    
    echo "=> Creating User for database: data"
    curl -s -k -X POST -d "q=CREATE USER ${INFLUXDB_DATA_USER} WITH PASSWORD '${INFLUXDB_DATA_PW}'" $(echo ${API_URL}'/query?u=root&p=root')
    curl -s -k -X POST -d "q=GRANT ALL ON data TO ${INFLUXDB_DATA_USER}" $(echo ${API_URL}'/query?u=root&p=root')
    
	echo "=> Creating User for database: grafana"
    curl -s -k -X POST -d "q=CREATE USER ${INFLUXDB_GRAFANA_USER} WITH PASSWORD '${INFLUXDB_GRAFANA_PW}'" $(echo ${API_URL}'/query?u=root&p=root')
    curl -s -k -X POST -d "q=GRANT ALL ON data TO ${INFLUXDB_GRAFANA_USER}" $(echo ${API_URL}'/query?u=root&p=root')
    
    echo ""
    
    echo "=> Changing Password for User: root"
    curl -s -k -X POST -d "q=SET PASSWORD FOR root = '${ROOT_PW}'" $(echo ${API_URL}'/query?u=root&p=root')
    echo ""
